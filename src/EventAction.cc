...
void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  */
  ...
  auto fillHits = [&](TrackerHitsCollection* hc) {
	...
	analysisManager->FillNtupleIColumn(0, 2, hit->GetPlaneID());
	analysisManager->FillNtupleIColumn(0, 3, hit->GetDetID());     // copy number
	analysisManager->FillNtupleDColumn(0, 4, hit->GetPos().x()/mm);
	analysisManager->FillNtupleDColumn(0, 5, hit->GetPos().y()/mm);
	analysisManager->FillNtupleDColumn(0, 6, hit->GetPos().z()/mm);
	analysisManager->FillNtupleDColumn(0, 7, hit->GetTime()/ns);
	analysisManager->FillNtupleDColumn(0, 8, hit->GetEdep()/keV);
